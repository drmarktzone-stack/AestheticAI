import type { DetectedRedFlag, RedFlagCode, RedFlagSeverity } from "./schema.ts";
import type { SymptomQuestionnaire } from "./schema.ts";
import type { z } from "zod";
import type { ImageSignalsSchema } from "./schema.ts";
...
/** Clinical rule-set — structured inputs only; free-text notes are never parsed here. */
const ANOMALY_RULES: AnomalyRule[] = [
  ...
  },
  {
    ...
    severity: "high",
    evaluate: ({ imageSignals }) =>
      (imageSignals?.reportedAsymmetryScore ?? 0) >= 0.75,
  },
  {
    code: "image_bruising_signal",
    severity: "high",
    evaluate: ({ imageSignals }) =>
      (imageSignals?.reportedBruisingCoverage ?? 0) >= 0.6,
  },
];
...
export function maxRedFlagSeverity(
